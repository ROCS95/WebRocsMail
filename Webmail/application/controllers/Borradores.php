...
class Borradores extends CI_Controller {
	...
	{
		$this->load->Model('Borradores_Model', 'BorradorM', 'default');
		$emails = $this->BorradorM->get_send();
	...
	function save() {
            ...
            if ($this->form_validation->run() == true)
            {
                ...
                if($id_email==="")
                {
                $id = $this->BorradorM->create($data);

                $this->session->set_flashdata('message', "Creado con éxito.");  
                }
                else
                {
                $id = $this->BorradorM->update($id_email,$data);
    ...
    function delete($id) {
            ...
            $this->load->Model('Borradores_Model', 'BorradorM', 'default');
                ...
                $Email = $this->BorradorM->get($id);
                $this->BorradorM->delete($id);
